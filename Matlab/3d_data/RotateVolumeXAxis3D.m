...
function RotateVolumeXAxis3D(fileNames, startAngle, endAngle, stepDegree)
    ...
    for f=1:size(fileNames,2)
        ...
        for angle = startAngle:stepDegree:endAngle
            ...
            trf = translateM_probe_center * rotateM_x / translateM_probe_center;%translateM_probe_center*rotateM_y*inv(translateM_probe_center)
            tform = affine3d(trf');
            ...
            trfFileName = strcat(directoryPath,'rotateM_x_x-axis-rotated_by_', int2str(angle),'_degrees.mat');
            save(trfFileName,'rotateM_x');

            %matrix of 2d slices for sequence
            %allocate
            slices = zeros(size(vol1, 2), size(vol1, 3), 3); %reduce computational cost by only reviewing 3 images, not frameNo
