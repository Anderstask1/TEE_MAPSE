...
for i = 1 : length(filePaths)
    ...
    PostProcessLandmarks3D(fileNames);

    %% MAPSE processing

    disp('Compute MAPSE values (plot mapse curve) (annulus render)');

    PostProcessMapse3D(fileNames);

    %% Save MAPSE results
    disp('Save MAPSE value results in table as a text file')

    SaveMapseValuesTxtFile(filesPath);
    %SaveMapseToCSV(filesPath);

    %% Plot statistics
    disp('Create plots of results from statistics')
    %mapse_4c_inf_reference_map
    %mapse_4c_ant_reference_map
    %mapse_2c_inf_reference_map 
    %mapse_2c_ant_reference_map
    %mapse_3c_inf_reference_map
    %mapse_3c_ant_reference_map
    ...
    %MitralAnnulus3DRendering(fileNames);
