...
%Author: gkiss
%Modified for 3D volumes: Anders Tasken
%Started 11.09.2020

function PostProcessMapseRecording3D(fileName, pathName, directoryPath, xlsfile, saveTrackingMovie, saveTrackingPlot, saveXls)

rootName = strcat(pathName, fileName);
...
%post-process for all rotated volumes
...
for i = 1 : numel(fields)
    ...
    %get field data
    imageData = hdfdata.RotatedVolumes.(fields{i}).images;
    ...
    mapseLeft = hdfdata.RotatedVolumes.(fields{i}).MAPSE_left_movement'.*pixelSize(2);
    mapseRight = hdfdata.RotatedVolumes.(fields{i}).MAPSE_right_movement'.*pixelSize(2);
    ...
    if saveTrackingMovie > 0
        recordingName = strcat(directoryPath, fileName,'_',fields{i});
        SaveMapseMovieAsMp4(recordingName, imageData, mapseLandmarks);
    end
    ...
    if saveTrackingPlot > 0
        ...
        %save figure to PNG
        imageName = strcat(directoryPath, fileName,'_',fields{i},'_MAPSE.png');
